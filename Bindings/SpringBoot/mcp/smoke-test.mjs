// Standalone MCP smoke test for the Nissth Bridge shim.
...
const client = new Client({ name: "nissth-smoke", version: "0.0.1" }, { capabilities: {} });
...
try {
  ...
  header("Nissth_Gateway / endpoint_lens against fixture");
  const fixtureRoot = path.resolve(__dirname, "..", "tests", "fixture", "src", "main", "java");
  ...
  header("Nissth_ReadReport / latest:endpoint_lens");
  const rr = await client.callTool({
    name: "Nissth_ReadReport",
    arguments: { relativePath: "latest:endpoint_lens", maxChars: 800 },
  });
  console.log(`isError=${rr.isError === true}`);
  console.log(snip(rr.content?.[0]?.text));
  if (rr.isError) failures++;

  header("Nissth_Verify / compilation against fixture");
  const ver = await client.callTool({
    name: "Nissth_Verify",
    arguments: {
      operation: "compilation",
      root_path: path.resolve(__dirname, "..", "tests", "fixture"),
    },
  });
