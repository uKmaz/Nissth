...
import { ComponentLens } from "../tools/ComponentLens";
import { DependencyAudit } from "../tools/DependencyAudit";
import { ExpoDoctorLens } from "../tools/ExpoDoctorLens";
import { RouteLens } from "../tools/RouteLens";
import { RouteScaffold } from "../tools/RouteScaffold";
...
async function main(argv: string[]): Promise<number> {
  ...
  if (descIdx >= 0) {
    ...
    if (manifest.raw.scope_extra_keys_doc?.[toolName]) {
      process.stdout.write(
        `\nscope.extra keys for ${toolName}:\n` +
          JSON.stringify(manifest.raw.scope_extra_keys_doc[toolName], null, 2) +
          "\n"
      );
    }
    return 0;
  ...
  const staleFlipper = new StaleFlipper(repoRoot);
  const dispatcher = new ToolDispatcher();
  const handlers: ToolHandler[] = [
    new RouteLens(reportWriter, staleFlipper, repoRoot),
    new ComponentLens(reportWriter, staleFlipper, repoRoot),
    new DependencyAudit(reportWriter, repoRoot),
    new ExpoDoctorLens(reportWriter, repoRoot),
    new RouteScaffold(reportWriter, repoRoot),
  ];
...
function parseFlagForm(argv: string[]): BridgeCommand {
  ...
  // CLAUDE.md §11.5 writes contract keys with hyphens (`--scope.max-depth`) while the
  // schema names them with underscores (`max_depth`). Accept both for the contract's own
  // keys; `scope.extra.*` keys are binding-defined and stay verbatim. (Phase 20, digest A3:
  // `--scope.root-path` used to fail schema validation with "must NOT have additional
  // properties" while `--describe` advertised `root_path`.)
