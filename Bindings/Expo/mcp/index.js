...
const GatewayInput = {
  command: z
    .object({
      tool: z.string().describe("Tool name, e.g. 'route_lens'"),
...
const VERIFY_OPS = {
  // Expo binding has no compile_verify analog; expo-doctor includes type-check
  // diagnostics among its battery of project-health checks.
  compilation: "expo_doctor_lens",
  doctor: "expo_doctor_lens",
...
const VerifyInput = {
  operation: z
    ...
    .describe(
      "Verification kind. 'compilation' and 'doctor' both → expo_doctor_lens; 'dependencies' → dependency_audit. The Expo binding has no dedicated compile_verify tool; expo-doctor's check battery is the project-health analog."
    ),
  root_path: z
    .string()
    .optional()
...
async function nissthReadReport({ relativePath, maxChars = 50000 }) {
  ...
  if (relativePath.startsWith("latest:")) {
    ...
    if (!tool) {
      return textResponse("`latest:` requires a tool name (e.g., 'latest:route_lens').", true);
    }
...
const server = new McpServer({ name: "nissth-bridge-expo", version: VERSION });

server.tool(
  "Nissth_Gateway",
  "Primary entry point for the Nissth Diagnostic Bridge expo binding. " +
    "Forwards a full JSON command per CLAUDE.md §11.2 to the CLI and returns the report inline. " +
    "Use this for any tool the binding exposes (route_lens, component_lens, dependency_audit, expo_doctor_lens, route_scaffold).",
  GatewayInput,
  nissthGateway
);
...
server.tool(
  "Nissth_Status",
  "Health probe: lists installed bindings, the N most recent Bridge reports, and whether the binding CLI is built.",
  StatusInput,
  nissthStatus
);
