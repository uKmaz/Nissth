...
| `Example Software Requirements Specification Report (3).pdf` | yes | yes |
| `Software Design Document Template.pdf` | yes | yes |
| `.claude/settings.local.json` | yes | yes |
| `AgentReports/Reports/2026-05-23_example-consumer-install-decisions.md` | yes | yes |
...
The orphan branch is built via `git worktree add --orphan -b nissth/public <scratchpad>/nissth-public`. The primary working directory at `<repo-root>` stays on `master` for the entire phase. The in-place `git checkout --orphan` variant is forbidden by §3.2 precisely because it would empty `Axiom/` from disk between Steps 5 and 11.
