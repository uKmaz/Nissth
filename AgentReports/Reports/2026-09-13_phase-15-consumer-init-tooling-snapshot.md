...
| Bake `DEFAULT_ROOT` into the launchers for `--wiring local` | keep launchers generic, require the env var | Matches the Example/Finans precedent; the launcher still honours the env var first and exports it so the dispatcher's tier-1 resolution agrees (F1). |
