...
supersedes:
  - none (extends the two candidates in 2026-05-23_unihub-consumer-install-decisions.md)
---
...
|:---|:---|:---|:---|:---|
| F1 | **Consumer launcher templates have no `NISSTH_FRAMEWORK_ROOT` fallback.** Both `Tools/nissth-bridge/consumer-launcher/*` hard-code `Tools/Nissth/…/dispatcher.js`; the comment promises the env var but the code never checks it. Every local-checkout consumer (UniHub ×2, Finans) hand-edited the launcher. | UniHub Report candidate #1 (2026-05-23), still open; Finans launchers written from scratch. | High — repeat offender | Phase 15 |
...
- The UniHub decision Report was reusable as-is: wiring decision took one paragraph, not a re-debate (§10's purpose, demonstrated).
