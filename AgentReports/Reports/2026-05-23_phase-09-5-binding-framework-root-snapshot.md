...
Harness at `<user-home>\AppData\Local\Temp\nissth-step16-smoke.mjs` (out-of-tree; not committed). Two tmpdir consumers (`CLAUDE.md` only, no `Bindings/`); `NISSTH_FRAMEWORK_ROOT=<repo-root>`.
...
### Step 17 — live smoke against Example-Frontend

Invocation:
```powershell
Push-Location '<workspace>\Example\src\Example-Frontend'
& '.\nissth-bridge.ps1' expo_doctor_lens
```

Result:
- Exit code: 0
- Launcher printed: `<workspace>\Example\src\Example-Frontend\AgentReports\Bridge\expo_doctor_lens_2026-05-23T104125Z.md`
- Report frontmatter present: `binding: expo`, `binding_version: 0.1.1`, `freshness.source` cites `subprocess: npx --yes expo-doctor in <workspace>\Example\src\Example-Frontend`, `source_state` cites subprocess exit 0 + stdout sha256 prefix.
