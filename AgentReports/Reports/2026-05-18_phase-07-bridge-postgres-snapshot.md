...
- **DBL `Summaries/_state.md` (Süprüz / future projects)** — the Postgres binding's `migration_status --mode auto` produces a clean digest of what's applied vs. failed; consumer projects can cite the latest Bridge report in plan §1 inputs without re-running `psql -c`.
