...
> **Companion** — the Example-Backend session's `StatusUpdate.md` entry at `<path>/Example\src\example-backend\AgentReports\StatusUpdate.md` (2026-05-23 22:13) is the upstream discovery record. This Report captures the framework-side close.
...
Tmpdir consumer at `/tmp/nissth-phase09-7-pg-smoke/` (CLAUDE.md only, no `Bindings/`); `NISSTH_FRAMEWORK_ROOT=<path>/Desktop/Nissth`. Invocation:
...
```powershell
Push-Location '<path>/Example\src\example-backend'
$env:NISSTH_PG_URL = 'postgresql://<db-user>:<REDACTED>@<db-host>/appdb?sslmode=no-verify'
...
Result:
- Exit code: **0**
- Report at `<path>/Example\src\example-backend\AgentReports\Bridge\migration_status_2026-05-23T214606Z.md`
- Report frontmatter: `binding: postgres`, `binding_version: 0.1.2`, `freshness.source: postgresql://appdb_user@<db-host>:5432/appdb` (password redacted per contract), `source_state: redo_lsn=125/970003E8` (proves a successful query — TLS handshake + `SELECT redo_lsn FROM pg_control_checkpoint()` both ran)
