...
- **Reference consumer:** `<path>/ExampleFinanceApp\DBL/**` at `620572c` — 11 real artifacts, all `design-only`; used as an external fixture in one test (skipped if the path is absent).
...
| `node Tools/dbl-check/check.mjs --root <path>/ExampleFinanceApp` | result | exit 0 today (11 design-only, no source); becomes exit 1 with 11× `design-only-source-exists` the moment Phase 01 creates `app/` |
...
- [x] **Step 9.** Run the checker on Nissth (`exit 0, scanned 0`) and on `<path>/ExampleFinanceApp` (`exit 0, scanned 11, 0 findings`); paste both outputs into the status entry. **Acceptance:** as stated.
...
- Editing any DBL artifact in the ExampleFinanceApp repo, even if the checker finds something there — report it in `Issues:` for that repo's session.
...
- [x] **Runtime/integration:** Step 9 on both repos. RESULT Nissth: 0 scanned exit 0; ExampleFinanceApp: 11 scanned, 0/0/0, exit 0.
...
**Verified:**
- [counts; Step 9 outputs on Nissth (0 scanned) and ExampleFinanceApp (11 scanned, 0 findings); freshness statement]
- Doc sync: [as §5]
...
**Next:**
- Phase_17 (docs) if approved; tell the ExampleFinanceApp session to add `node <framework>/Tools/dbl-check/check.mjs` to its Phase 01 §4.2.
```
