...
- **DBL:** none in this repo (`DBL/**` here is templates only). The consumers' maps — `ExampleDesktopApp/DBL/DependencyMaps/layers.md` and `ExampleFinanceApp/DBL/DependencyMaps/layers.md` — are read as **input shapes**, not as authority over this repo; they are cited with their project prefix so the path is not read as one of this repo's own.
...
|:---|:---|:---|:---|:---|
| 1 | Read a real dependency map's boundary rules | read `ExampleDesktopApp/DBL/DependencyMaps/layers.md` and the ExampleFinanceApp equivalent | the two live consumers | The template shows bullets; what gets written may differ, and the linter must parse what exists |
...
| 4 | Establish the defect this is aimed at | re-read the 2026-09-13 assessment L2 and ExampleDesktopApp's Phase 06 note | the assessment | A linter aimed at the wrong defect is worse than none |
...
| a plan that targets files under a constrained boundary | signal | nothing; the conflict surfaces at execution (ExampleDesktopApp Phase 06) |
...
| 2 | **`Phase_19` got a waiver rather than an edit.** Its §1.1 lists three consumer artifacts on one line, the last two inheriting the `ExampleFinanceApp/` prefix from the first; the linter reads the third as a path in this repo. | The plan is closed and its sentence is correct English. Rewriting a closed plan's evidence line to satisfy a tool is the wrong direction; the waiver records the reason where a reader will meet it. This phase's own plan took the other fix — its citation was reworded to carry the project prefix, because it was still being written. |
...
**Result, 2026-09-25 00:40:** `plan-lint` **14/14**, `nissth-init` **32/32** (baseline 29), `dbl-check` 24/24, `public-cut` 12/12, dispatcher 32/32, `doc-claims` exit 0 — five suites, none regressed. `plan-lint` over this repo: **23 plans, 0 error, 0 warn, 2 info** (both `no-step-targets`, both correct — those two plans' §3 is prose and verification). Over the consumers, where the dependency maps live: **ExampleDesktopApp 19 plans → 5 `dependency-map-not-cited`**, all in plans 13–18, and **every plan 01–12 cites the map** — the lapse is real, dated, and exactly the defect the check was built for; **ExampleFinanceApp 9 plans → 1**, a Swift target file under a `targets/**` boundary. `nissth-init --check` on both consumers: drift reported (this session's own tree rows), feedback section silent because that digest's rows were all closed in Phase 22 — the positive case is covered by three fixture tests instead.
