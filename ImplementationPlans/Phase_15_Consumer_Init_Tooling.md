...
- [x] **Step 2.** Write `templates/CLAUDE.banner.md` — the FinansYönetimApp head (title line + two blockquote paragraphs) generalised with `{{NAME}}`, `{{DATE}}`, `{{STACK_SENTENCE}}`, `{{FRAMEWORK_ROOT}}`. **Acceptance:** rendering with Finans values reproduces `FinansYönetimApp/CLAUDE.md:1-5` modulo the project-specific status sentence.
