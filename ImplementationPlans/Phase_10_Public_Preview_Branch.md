...
- [x] **Step 5.** Remove the non-shareable trees and files **from the worktree only**. **Files (all paths relative to `<scratchpad>/nissth-public/`):** `Axiom/` (recursive), `Süprüz Software Requirements Specification Report (3).pdf`, `Software Design Document Template.pdf`, `.claude/settings.local.json`, `AgentReports/Reports/2026-05-23_unihub-consumer-install-decisions.md`. **Operation:** remove. **Acceptance:** none of the five paths exists **inside the worktree**, AND `C:\Users\admin\Desktop\Nissth\Axiom\` is verified still present with 148 files immediately after this step.
