...
- **Consumers:** `PostPilot/AgentReports/StatusUpdate.md` (the repeated `AgentReports/Archive/` item, first raised 2026-09-19), the Nissth ledger entry 2026-09-13 08:22 item 2 (the init handoff PostPilot asked for, never delivered — `Phase_19` became the Bridge flip fix instead), `FinansYönetimApp/AgentReports/Reports/2026-09-13_nissth-feedback-digest.md` row A4.
...
| 2 | Confirm `nissth-init` never creates `Archive/` | read the keeper list | `init.mjs:161-163` | PostPilot's claim, verified at the source |
...
|:---|:---|:---|:---|
| Does PostPilot's copy differ from the framework body? | yes, by the Phase 20 hunks | **worse — 1010 of 1164 body lines differ, first at line 7.** It is behind phases 17, 18, 20 and 21, so everything after the first insertion is offset. It was also missing `Tests/README.md`'s sibling rule and had never received §1's `git fetch` | no — worse than expected |
| Does FinansYönetimApp's copy differ? | yes, by the Phase 21 hunks only (Phase 20 re-synced it) | yes — 37 lines, first at body line 1010 (the Phase 21 tool descriptions), plus two missing skeleton paths | yes |
...
- [x] **Step 8.** Re-sync both consumers' `CLAUDE.md` framework bodies using `--check` as the verifier, and append a status entry to each consumer's own ledger. **Files:** `PostPilot/CLAUDE.md`, `FinansYönetimApp/CLAUDE.md` + each `AgentReports/StatusUpdate.md`. **Operation:** modify. **Acceptance:** `--check` exits 0 for both; each consumer's banner is untouched; each repo has a new entry naming what changed.
...
| 3 | **Step 8 also filled the missing skeleton paths** (`Archive/README.md` in both consumers, `Tests/README.md` in FinansYönetimApp), which the step described only as a `CLAUDE.md` re-sync. | Leaving `--check` at exit 1 in both consumers on the day it shipped would have taught its first two readers to ignore it. |
...
**Result, 2026-09-24 23:15:** `node --test Tools/nissth-init/test.mjs` **29 pass / 0 fail** (baseline 21); `node Tools/doc-claims/validate.mjs` exit 0; `node --test Tools/dbl-check/test.mjs` 24/24 and `node --test Tools/nissth-bridge/test.mjs` 32/32, both unchanged. `--check` before the re-sync: PostPilot exit 1 (1010 lines), FinansYonetimApp exit 1 (37 lines + 2 missing paths); after: **both exit 0, in sync**. A non-project directory exits 2. Each consumer has one new status entry, an unchanged banner, and a commit (PostPilot `ca14140`, FinansYonetimApp `a716d7c`).
