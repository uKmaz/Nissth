...
- **DBL:** none in this repo (`DBL/**` here is templates only). The consumers' artifacts are read as **evidence of the defect**, not as authority: `FinansYonetimApp/DBL/**` (17 artifacts) and `PostPilot/DBL/**` (35).
...
| 3 | Measure the blast radius | for every artifact whose ref `HEX_REF` rejects, extract the ref by hand and run the same `git diff` the tool would | FinansYonetimApp | Turns "the check may be blind" into a number |
...
| Does `covers-changed-since` run on all of them? | yes | **no** — `HEX_REF` anchors `^…$`, so only the bare form is ever checked. All **17** FinansYonetimApp artifacts use another form; not one has ever had its freshness checked | no |
...
| FinansYonetimApp `dbl-check --strict` | verdict | `0 error, 0 warn, 0 info` while 4 artifacts are stale |
...
| FinansYonetimApp `dbl-check` | verdict | the 4 stale artifacts are named |
...
**Result, 2026-09-25 01:15:** six suites green — `dbl-check` **27/27** (baseline 24), `dbl-regen` **11/11** (new), `plan-lint` 14/14, `nissth-init` 32/32, `public-cut` 12/12, dispatcher 32/32. `doc-claims` exit 0; `plan-lint` over this repo 24 plans, 0 errors. **The count agreed:** the fixed `dbl-check` named exactly the four artifacts §1.3 measured by hand, with the same file counts (74, 35, 6, 1) — no more and no fewer. PostPilot's clean verdict was cross-checked by hand rather than trusted (9 commits since `ec8d033`, none touching `src/**` or `Tests/**`). Worksheets printed for two real stale artifacts. Both consumers re-synced and `--check`-verified; neither consumer's `DBL/**` was modified.
...
- [x] **Runtime/integration:** fixed `dbl-check` over both consumers — expected: FinansYonetimApp names exactly the 4 artifacts §1.3 found, PostPilot's verdict explained either way. `dbl-regen --artifact` worksheets for two of them.
