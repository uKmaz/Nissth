...
- **Estimated scope:** Three tool defects, all of them raised by the FinansYönetimApp consumer and all reproduced against that repo before this plan was authored. Two files of binding source (`Bindings/Expo/src/tools/ExpoDoctorLens.ts`, `…/DependencyAudit.ts`), one framework tool (`Tools/dbl-check/check.mjs`), their tests, the Expo manifest (one new mode), and three READMEs. No contract change — `Bindings/_schemas/` is untouched.
...
- **Bridge reports:** the consumer's `FinansYönetimApp/AgentReports/Bridge/expo_doctor_lens_2026-09-20T101924Z.md` (generated_at 2026-09-20T10:19:24Z, "Checks parsed: 0") and `…/dependency_audit_2026-09-20T101918Z.md` (same run, six `@/…` rows as `missing`, three config-file deps as `dev_in_prod`). Cited as the field evidence of the defects, not as live state for this repo.
...
|:---|:---|:---|:---|:---|
| 1 | Capture the real expo-doctor output, both default and verbose | `npx --yes expo-doctor` / `… --verbose` | run in the consumer `FinansYönetimApp` | The digest says "parses 0 checks"; the fix depends on *why*, which only the raw output shows |
...
| Baseline suites | Expo 14 suites / 67 tests; dbl-check green | Expo 14/67 PASS. `Tools/dbl-check/test.mjs` **20 pass / 1 fail** — not green: `FinansYönetimApp consumer: 11 design-only artifacts` asserts a hard-coded count against a live consumer that now holds 17 (`dbl-check --root <consumer>` → 17 scanned, 0/0/0). Pre-existing, unrelated to this plan's three defects, in a file this plan edits → folded in as Step 7b | no — see below |
...
- [x] **Step 7b.** Un-couple the `dbl-check` suite from one host's consumer checkout. The test `FinansYönetimApp consumer: 11 design-only artifacts, currently clean` asserts a hard-coded artifact count against a live consumer repo at a hard-coded `C:\Users\admin\…` path; the consumer has since grown to 17 and the test fails on the current tip. **File:** `Tools/dbl-check/test.mjs:13,243-247`. **Operation:** modify. **Acceptance:** the path comes from `NISSTH_CONSUMER_ROOT` with the known checkout as fallback, the assertion is "every artifact parses, zero findings" with no count, and the case skips cleanly on a host without the checkout.
