...
>
> **Status:** Framework operational. Three stack bindings ship against one shared command contract — §8.1 Spring Boot, §8.2 Expo, §8.3 PostgreSQL — behind the unified `nissth-bridge` dispatcher (§11.15), with framework-root resolution so a consumer project can install Nissth as a submodule. Five repo checks back the rules that discipline alone kept missing: `nissth-init` (§9.1), `doc-claims` (§12), `dbl-check` (§13), `plan-lint` (§14), `dbl-regen` (§15). Not built yet: Claude Code hook-based enforcement under `.claude/`. The development record — how each of these came to exist, and what it cost to find — is in `ImplementationPlans/` and `AgentReports/`.
...
├── Tests/                          ← Test sources + verification artifacts. Consumer test projects live HERE;
│                                      never a `tests/` or `test/` sibling (case-insensitive filesystems merge them).
...
cross the threshold had to invent it (ExampleDesktopApp, 2026-09-19).
...
2. **Bootstrap (mechanical, plan-exempt).** Run `node Tools/nissth-init/init.mjs --target <dir> --name <ProjectName> --stack <expo|spring-boot|postgres|none> [--wiring local|submodule]` from the Nissth checkout (`Tools/nissth-init/README.md`). It creates, and only creates: `CLAUDE.md` (project banner + this framework body verbatim), `AGENTS.md`, `ImplementationPlans/_TEMPLATE.md`, `AgentReports/StatusUpdate.md` (schema preamble + a filled "Bootstrap" entry), `AgentReports/{Reports,Bridge,Snapshots}/`, `DBL/{Summaries,DependencyMaps,APIIndex,SchemaIndex}/_TEMPLATE.md`, `Tests/README.md` (the test-sources rule), `Tools/`, `.claude/settings.json`, `.gitignore`, `.gitattributes`, and the two `nissth-bridge` launchers — LF-normalised, refusing to overwrite any existing file, running no subprocess. No source code. The "Bootstrap" status entry it writes is the only execution allowed without an approved plan, and only because there is no source code to modify yet. `git init`, dependency installs, and SRS/SDD remain the agent's (steps 1, 3). Greenfield projects (no source yet) run Phase 00 in design-only mode — §7.6.
   **The run ends with a handoff: open the next session *in the target directory*.** An agent that keeps driving the new project from the framework checkout boots the framework's ledger instead of the consumer's, and every shell call returns to the framework's directory — which is exactly what happened to the first consumer initialised this way (ExampleDesktopApp, phases 00–06).
...
|:---|:---|:---|
| **Decision record** | An architecture or design choice has options the team would otherwise re-debate later | "Why JPA over JOOQ", "Why Modular Monolith over microservices", "Why a payment provider over Stripe" |
...
| **Spec digest** | A long external spec (PDF, RFC, vendor doc) needs a project-tailored summary | "a payment provider API surface used by Example", "Apple SSO requirements digest" |
...
3. **Spec ingestion.** When the agent reads a long external spec (PDF, RFC, vendor doc) and bases plans on it, a `spec_digest` Report is authored so future agents do not re-extract from the source. Example's SRS and SDD intake on 2026-05-05 is the canonical example — the markdown derivatives in `ImplementationPlans/SRS.md` and `SDD.md` are the digest; a `spec_digest` Report can additionally summarize "what changed between revisions."
...
nissth-bridge endpoint_lens \
  --mode full \
  --scope.package com.example.reservation \
  --scope.max-depth 2 \
  --output.destination file

# JSON stdin form (same payload as MCP):
echo '{"tool":"endpoint_lens","scope":{"package":"com.example.reservation"}}' | nissth-bridge --json-stdin
...
- Consumer projects (e.g., the Example project at `Desktop/ExampleApp/`) pull a binding in by git submodule, version-pinned dependency, or `gradle includeBuild`. The binding's source does not get copied into the consumer.
...
nothing enforced either. The first greenfield consumer (ExampleFinanceApp, Phase 00,
